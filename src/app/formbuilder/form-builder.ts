...
const FormBuilder =  (opts, element) =>{
  ...
  let $stage = (<any>$(d.stage));
    ...
    let fieldOptions = function (fieldData) {
      ...
      let optionActions = [
        m('a', i18n.addOption, {className: 'add-'+name+' add-opt'})
      ];
      let fieldOptions = [
    ...
    const defaultFieldAttrs = type => {
        ...
        const typeAttrsMap = {
            ...
            date: defaultAttrs.concat(['qrCode','addTask']),
            "datetime-local": defaultAttrs.concat(['qrCode','addTask']),
            time: defaultAttrs.concat(['qrCode']),
        ...
        if (utils.inArray(type, ['header', 'paragraph', 'button','newline','section'])) {
    ...
    let advFields = values => {
        ...
        const advFieldMap = {
          ...
          //  style: () => btnStyles('default',values),
            bulkAdd:()=>bulkAddMethod('default',values),
            ...
            qrCode:()=>boolAttribute('qrCode',values,{
                first: ' ',
                second: i18n.addQR
            }),
            addTask:()=>boolAttribute('addTask',values,{
                first: ' ',
                second: i18n.addTaskCreation
            })
           // multiple:() =>{}
    ...
    const bulkAddMethod = (style,values) => {
        let styleField: any = '';
        ...
        let styleLabel = `<label>${i18n.addOptionButtonName}</label>`;
        styleField += h.input({
            value: style || 'default',
            type: 'hidden',
            className: 'btn-style'
        ...
        styles.btn.forEach(btnStyle => {
            ...
            let btn = m('button', i18n.addOptionButton, {
                value: btnStyle,
                type: 'button',
                className: 'btn-drag-and-drop bulkAddButton',
                id:'bulkAdd'+values.name,
            }
            )
            styleField += btn.outerHTML;
        });
      
        styleField += '</div>';
        styleField += ` <div id="modal` + values.name + `" class="bulkmodal">
                    <div class="bulkmodal-content">
                        <span id="close-`+ values.name + `" class="bulkmodalclose">&times;</span>
                        <div class="app-modal-header">
                        <h4 style="text-align: center;" class="modal-title">Bulk Add Options</h4>
                      </div>
                      <div class="app-modal-body">
                      <p><span class="asterisk">Note:</span>Please add content and press Submit</p>
                      <textarea style="width: 100%;" rows="10" name="name-`+ values.name + `"
                        id="textparse-`+ values.name + `"></textarea><br><br>
                      <input style="margin-left: 50%;" id="`+ values.name + `" class="btn btn-success multi-select-options" type="button"
                        value="Submit">
                    </div>
                    </div>
    ...
    const textAttribute = (attribute, values) => {
        ...
        if (!noMakeAttr.some(elem => elem === true)) {
            ...
            if (attribute === 'label') {
                ...
                attributefield += `<input ${utils.attrString(inputConfig)}`+(attribute==='name'?'readonly':'')+`>`;
            }
    ...
    let selectFieldOptions = function (name, optionData, multipleSelect,labelValue?) {
        let optionInputType = {
        ...
        if(labelValue){
            optionTemplate.label=labelValue;
            optionTemplate.value=labelValue;
        }
    ...
    // delete options
      ...
      // touch focus
    $stage.on('click', '.bulkAddButton', e => {
        let id: string = e.currentTarget.id
        let name = id.replace('bulkAdd', '');
        $('#textparse-' + name).val("");
    ...
    $stage.on('change', '.fld-multiple', e => {
        let _event: any = e;
        let newType : any = _event.target.checked ? 'radio' : 'radio';
  ...
  $stage.on('click', 'input.fld-access', function (e) {
    ...
    $stage.on('click', '.multi-select-options', function (e) {
        e.preventDefault();

        let $optionWrap = $(".add-" + e.target.id).closest('.field-options');
        var dataList:any = $('#textparse-' + e.target.id).val();

        if (dataList) {
            dataList.split('\n').forEach(element => {
                if (element != '')
                    $('.sortable-options', $optionWrap)
                        .append(selectFieldOptions(e.target.id, false,e.target.id.includes("radio")?false:true, element));
            });
        }
        $('#close-' + e.target.id).click();
    });
