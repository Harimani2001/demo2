...
@Component({
  ...
  styleUrls: ['./electronic-signature.component.css','./../../../../node_modules/sweetalert2/dist/sweetalert2.min.css'],
...
export class ElectronicSignatureComponent implements OnInit {
  ...
  constructor(public router: ActivatedRoute,public service: ElectronicSignatureService,
    public fb: FormBuilder, public esignAgreementMessage: EsignAgreementMessege) { }
  ...
  ngOnInit() {
    ...
    this.service.postApi(this.receivedId, 'externalApproval/loadPdfPreviewforExternalApproval').subscribe(response => {
      this.viewpdf = true;
  ...
  sendOtp(){
    ...
    if(this.esignForm.get('userName').value){
      ...
      this.service.getApi(url).subscribe(response => {
  ...
  onSubmit() {
    ...
    if (this.esignForm.valid) {
      ...
      this.service.getApi(url).subscribe(response => {
        ...
        if(response){
          this.service.postApi(dto,"externalApproval/approveDocument").subscribe(response => {
